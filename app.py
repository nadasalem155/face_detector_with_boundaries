...
def draw_boxes(bgr_img: np.ndarray, boxes):
    ...
    for (x1, y1, x2, y2, score) in boxes:
        ...
        # Draw black stroke
        cv2.putText(out, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,0), 4)
        # Draw green text over it
...
def handle_image(pil_img: Image.Image, source: str, conf: float, max_width: int):
    ...
    st.download_button(
        "Download result (PNG)",
        data=buf.getvalue(),
        file_name="faces_detected.png",
        mime="image/png",
        key=f"download_{source.replace(' ', '_')}"  # unique key for each button
    )
